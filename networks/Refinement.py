...
class RefineBlocks(nn.Module):

    def __init__(self, in_channels, increment, num_layers, use_dense):

        super(RefineBlocks, self).__init__()
        ...
        self.feature_fusion_local = nn.Conv3d(in_channels + increment * num_layers, increment, 1, 1, 0)\
            if use_dense else nn.Conv3d(in_channels, in_channels, 1, 1, 0)

    def forward(self, x):

        return x + self.feature_fusion_local(self.layers(x))
...
class Refinement(nn.Module):
    ...
    def __init__(self, num_channels=128, base=64, num_blocks=4, num_layers=3, use_dense=True):
        ...
        self.blocks = nn.ModuleList([RefineBlocks(base, base, num_layers, use_dense) for _ in range(num_blocks)])

        # global feature fusion
        self.feature_fusion_global = nn.Sequential(
            nn.Conv3d(base * num_blocks, base, 1, 1),
